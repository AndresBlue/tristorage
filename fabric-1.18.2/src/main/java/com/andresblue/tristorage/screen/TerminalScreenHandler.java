...
public final class TerminalScreenHandler extends ScreenHandler {
    ...
    public boolean onButtonClick(PlayerEntity player, int id) {
        ...
        int nextPage = switch (id) {
            case 0 -> Math.max(0, page - 1);
            case 1 -> Math.min(core.pageCount(PAGE_SIZE) - 1, page + 1);
            default -> page;
        };
        if (nextPage != page) {
            page = nextPage;
        ...
        return new PropertyDelegate() {
            ...
            public int get(int index) {
                ...
                return switch (index - PAGE_SIZE) {
                    case 0 -> page;
                    case 1 -> core.pageCount(PAGE_SIZE);
                    case 2 -> core.storedTypes();
                    case 3 -> (int) Math.min(Integer.MAX_VALUE, core.totalItems());
                    default -> 0;
                };
            }
