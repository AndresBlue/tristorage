...
public final class StorageCoreBlockEntity extends BlockEntity implements NamedScreenHandlerFactory {
    ...
    public StorageCoreBlockEntity() {
        super(TriStorageMod.STORAGE_CORE_BLOCK_ENTITY);
    }

    public StorageTier tier() {
        if (getCachedState().getBlock() instanceof StorageCoreBlock) {
            return ((StorageCoreBlock) getCachedState().getBlock()).tier();
        }
    ...
    public Text getDisplayName() {
        return new TranslatableText("screen.tristorage.core", tier().level());
    }
    ...
    public NbtCompound writeNbt(NbtCompound nbt) {
        ...
        for (StoredEntry entry : entries.values()) {
            ...
            stored.put("Stack", entry.template.writeNbt(new NbtCompound()));
    ...
    public void fromTag(BlockState state, NbtCompound nbt) {
        ...
        entries.clear();
        totalItems = 0;
        NbtList list = nbt.getList(ENTRIES_KEY, 10);
    ...
    public static final class EntryView {
        ...
        public EntryView(String key, ItemStack stack, long count) {
            this.key = key;
            this.stack = stack;
            this.count = count;
        }
